...
namespace Moadian.API
{
    public class Api
    {
        ...
        public async Task<TokenModel> GetToken()
        {
            ...
            var json = Newtonsoft.Json.Linq.JObject.FromObject(response);
            var data = json["result"]?["data"] ?? json["data"];

            if (data == null)
                return null;

            var tokenStr = data["token"]?.ToString();
            var expiresIn = data["expiresIn"]?.ToObject<int?>() ?? 0;

            if (string.IsNullOrEmpty(tokenStr))
                return null;

            return new TokenModel(tokenStr, expiresIn);
        }
